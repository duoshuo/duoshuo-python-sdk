...
class Command(BaseCommand):
    def handle(self, *args, **options):
        if not args:
            raise CommandError('Tell me what\'s data you want synchronization (user/article/comment)')
        ...
        if args[0] == 'user':
            ...
            for comment in comments:
                ...
                data['posts[%s][source]'% comment.id] = 'import',
                try:
                    thread_id = threads['%s_%s_%s' % (comment.content_type.app_label,comment.content_type.model,comment.content_object.id)]
                except:
                    pass
                else:
            ...
            _s = raw_input('Please input the article model name such as `posts.article`:')
            if len(_s.split('.')) != 2:
            ...
            article_model = get_model(app_label, model_name)
            if not article_model:
                raise CommandError('Cant\'t find model: %s.' % _s)

            try:
                article_model.get_absolute_url
            except AttributeError:
                raise CommandError('Please define a get_absolute_url() method.')

            thread_schema = {'title': '', 'content': ''}
            thread_schema['title'] = raw_input('Enter article title filed name: ')
            thread_schema['content'] = raw_input('Enter article content filed name: ')

            threads = article_model.objects.all()
